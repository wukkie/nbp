...
public class NBPRequestParser {
	...
	private static Date parseDate(String date) throws IllegalArgumentException {
		...
		try {
			...
			throw new IllegalArgumentException(String.format("Format daty '%s' jest błędny. Prawidłowy format to %s",
					date, NBPApplicationConfig.ARG_DATE_FORMAT));
		}
		return validDate;
	}
	...
	public static NBPRequest parseArgs(String[] args) throws IllegalArgumentException {
		validate(args);
		return new NBPRequestBuilder().setTable(getTable(null)).setType(getType(null)).setFormat(getFormat(null))
				.setDate(parseDate(args[0])).build();
	}
}
